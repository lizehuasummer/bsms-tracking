...
def step3_fill_excel(func_issues, bug_issues, notes_cache):
    ...
    bug_headers = ["系统", "bug号", "bug名称", "priority", "status", "持续时间",
                     "创建人", "创建时间", "待修复人", "待修复时间", "测试人", "测试时间", "完成人", "完成时间"]
    for ci, h in enumerate(bug_headers, 1):
        ws3.cell(1, ci).value = h
    ...
    for issue in bug_issues:
        ...
        ws3.cell(bug_row, 6).value = duration
        ws3.cell(bug_row, 7).value = issue.get("author", {}).get("username", "")
        ws3.cell(bug_row, 8).value = fmt_time(issue.get("created_at"))

        if fix_sc:
            ws3.cell(bug_row, 9).value = fix_sc["user"]
            ws3.cell(bug_row, 10).value = fmt_time(fix_sc["time"])
        if test_sc:
            ws3.cell(bug_row, 11).value = test_sc["user"]
            ws3.cell(bug_row, 12).value = fmt_time(test_sc["time"])
        if done_sc:
            ws3.cell(bug_row, 13).value = done_sc["user"]
            ws3.cell(bug_row, 14).value = fmt_time(done_sc["time"])
